...
- 当前内置角色固定为 4 类：
  ...
  - `docent` / 讲解员：保持原只读讲解类权限不变，默认仅保留 `工作台 / 文物列表 / 文物详情 / 库存查询 / 已修复 / 个人中心` 等低风险入口，不保留审批、派发和编辑删除类按钮
...
主布局由左侧权限菜单、顶部用户导航、顶部标签页导航和主内容区组成。左侧菜单来自 `GET /api/auth/menus`，会按当前用户角色权限选择性显示；顶部标签页融合在用户与退出登录所在的顶部卡片容器内，“首页”标签固定保留且不可关闭，其余业务标签可关闭并支持来回切换。
