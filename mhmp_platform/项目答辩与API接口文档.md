...
- 当前内置角色固定为 4 类：
  ...
  - `docent` / 讲解员：保持原只读讲解类权限不变，默认仅保留 `首页 / 文物列表 / 文物详情 / 库存查询 / 已修复 / 个人中心` 等低风险入口，不保留审批、派发和编辑删除类按钮
...
- 当前系统内所有实际人员选择框（出库经手人、盘点负责人、转存负责人）统一展示为“姓名（账号）”，并按账号升序排序，便于多账号场景下快速区分同名人员；其余角色、状态、字典或日志账号筛选不属于人员指派选择框
- 当前默认权限边界已经同步到本地数据库：
  - `researcher` 默认拥有 `文物列表 / 新增文物 / 文物入库 / 文物出库 / 库存查询 / 我的转存 / 盘点任务 / 我的盘点 / 待修复文物 / 我的修复 / 已修复` 及对应业务提交按钮
  - `docent` 默认仅保留只读相关页面，不再拥有 `新增文物 / 文物入库 / 文物出库 / 出库归还 / 我的转存 / 我的盘点 / 修复审批 / 我的修复` 等高风险入口
- 运行时以数据库中的 `sys_role_menu` 实际授权为准；`repair_base_permissions_and_menus.sql` 提供的是内置角色默认模板。如果后续在“角色管理”里手动调整授权，应以你调整后的实际数据库权限为准
...
主布局由左侧权限菜单、顶部用户导航、顶部多标签页导航和主内容区组成。左侧菜单来自 `GET /api/auth/menus`，会按当前用户角色权限选择性显示；顶部标签页在点击功能页面时自动生成，“首页”标签固定保留且不可关闭，其余业务标签可关闭并支持来回切换。

| 前端路由 | 页面功能 | 主要权限 |
| --- | --- | --- |
| `/login` | 登录页 | 无 |
| `/dashboard` | 首页介绍 | `dashboard:view` |
...
| 方法 | 路径 | 权限 | 说明 | 关键参数 |
| --- | --- | --- | --- | --- |
| GET | `/api/dashboard/home` | `dashboard:view` | 获取首页介绍内容 | 无 |
